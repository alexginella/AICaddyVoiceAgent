...
2. **Course Guide Service** (FastAPI, separate process) receives `POST /ensure-guide`: lookup-first checks for `agent/data/courses/{slug}.pdf` and a non-empty Chroma collection; if missing, generates a yardage-book PDF and indexes it. The UI waits for `status: "ready"` before allowing a call.
...
- **Default** (no course name): Static `agent/data/yardage_book.pdf`, indexed at agent startup if present.
- **Per-course yardage book**: Generated by the **Course Guide Service** before the call—professional yardage-book-style text (layout and yardages only), PDF at `agent/data/courses/{slug}.pdf`, indexed into Chroma collection `course_{index_slug}` (shared `agent/vector_store/` with the agent process).
